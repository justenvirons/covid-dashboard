...
DemoChars_Chicago <- DemoChars_Chicago %>% select(-1, -Record.ID) %>% filter(Geography=="Chicago") %>% rename("Total"="Population...Total", "Age0_17"="Population...Age.0.17", "Age18_29"="Population...Age.18.29", "Age30_39"="Population...Age.30.39", "Age40_49"="Population...Age.40.49", "Age50_59"="Population...Age.50.59", "Age60_69"="Population...Age.60.69", "Age70_79"="Population...Age.70.79", "Age80Pl"="Population...Age.80.", "Female"="Population...Female", "Male"="Population...Male", "Latinx"="Population...Latinx", "AsianNL"="Population...Asian.Non.Latinx", "BlackNL"="Population...Black.Non.Latinx", "WhiteNL"="Population...White.Non.Latinx", "OthRaceNL"="Population...Other.Race.Non.Latinx")
...
Chicago_COVID19_CasesDeathsHospitalizations <- Chicago_COVID19_CasesDeathsHospitalizations %>% rename("Cases"="Cases - Total","Deaths"="Deaths - Total","Hosp"="Hospitalizations - Total","Cases_Age0_17"="Cases - Age 0-17","Cases_Age18_29"="Cases - Age 18-29","Cases_Age30_39"="Cases - Age 30-39","Cases_Age40_49"="Cases - Age 40-49","Cases_Age50_59"="Cases - Age 50-59","Cases_Age60_69"="Cases - Age 60-69","Cases_Age70_79"="Cases - Age 70-79","Cases_Age80Pl"="Cases -  Age 80+","Cases_UnkAge"="Cases - Age Unknown","Cases_Female"="Cases - Female","Cases_Male"="Cases - Male","Cases_UnkGend"="Cases - Unknown Gender","Cases_Latinx"="Cases - Latinx","Cases_AsianNL"="Cases - Asian Non-Latinx","Cases_BlackNL"="Cases - Black Non-Latinx","Cases_WhiteNL"="Cases - White Non-Latinx","Cases_OthRaceNL"="Cases - Other Race Non-Latinx","Cases_UnkRace"="Cases - Unknown Race/Ethnicity","Deaths_Age0_17"="Deaths - Age 0-17","Deaths_Age18_29"="Deaths - Age 18-29","Deaths_Age30_39"="Deaths - Age 30-39","Deaths_Age40_49"="Deaths - Age 40-49","Deaths_Age50_59"="Deaths - Age 50-59","Deaths_Age60_69"="Deaths - Age 60-69","Deaths_Age70_79"="Deaths - Age 70-79","Deaths_Age80Pl"="Deaths - Age 80+","Deaths_UnkAge"="Deaths - Age Unknown","Deaths_Female"="Deaths - Female","Deaths_Male"="Deaths - Male","Deaths_UnkGend"="Deaths - Unknown Gender","Deaths_Latinx"="Deaths - Latinx","Deaths_AsianNL"="Deaths - Asian Non-Latinx","Deaths_BlackNL"="Deaths - Black Non-Latinx","Deaths_WhiteNL"="Deaths - White Non-Latinx","Deaths_OthRaceNL"="Deaths - Other Race Non-Latinx","Deaths_UnkRace"="Deaths - Unknown Race/Ethnicity","Hosp_Age0_17"="Hospitalizations - Age 0-17","Hosp_Age18_29"="Hospitalizations - Age 18-29","Hosp_Age30_39"="Hospitalizations - Age 30-39","Hosp_Age40_49"="Hospitalizations - Age 40-49","Hosp_Age50_59"="Hospitalizations - Age 50-59","Hosp_Age60_69"="Hospitalizations - Age 60-69","Hosp_Age70_79"="Hospitalizations - Age 70-79","Hosp_Age80Pl"="Hospitalizations - Age 80+","Hosp_UnkAge"="Hospitalizations - Age Unknown","Hosp_Female"="Hospitalizations - Female","Hosp_Male"="Hospitalizations - Male","Hosp_UnkGend"="Hospitalizations - Unknown Gender","Hosp_Latinx"="Hospitalizations - Latinx","Hosp_AsianNL"="Hospitalizations - Asian Non-Latinx","Hosp_BlackNL"="Hospitalizations - Black Non-Latinx","Hosp_WhiteNL"="Hospitalizations - White Non-Latinx","Hosp_OthRaceNL"="Hospitalizations - Other Race Non-Latinx","Hosp_UnkRace"="Hospitalizations - Unknown Race/Ethnicity")
...
Chicago_COVID19_CasesDeathsHospitalizations_pct <- table %>% arrange(Date) %>% mutate_at(vars(5:22), fx_pctcas) %>% mutate_at(vars(23:40), fx_pctdth) %>% mutate_at(vars(41:58), fx_pcthos) %>% mutate_if(is.numeric,list(~replace_na(.,0))) %>% ungroup()
...
Chicago_COVID19_CasesDeathsHospitalizations_pct_long <- Chicago_COVID19_CasesDeathsHospitalizations_pct_long %>% mutate(WeekNo = as.numeric(format(Date,"%W"))+1, EndDate = ceiling_date(Date,unit="week"))  %>% mutate(WeekNo = ifelse(format(Date,"%Y")==2021,WeekNo+52,WeekNo)) %>% mutate(WeekNo=WeekNo-3)
# test <- Chicago_COVID19_CasesDeathsHospitalizations_pct_long %>% group_by(EndDate) %>% summarise(n(),min(WeekNo  ))
# as.numeric(format(as.Date("2020-03-08"),"%W")) - as.numeric(format(as.Date("2020-01-22"),"%W"))

# Chicago_COVID19_CasesDeathsHospitalizations_pct_long$WeekNo <- Chicago_COVID19_CasesDeathsHospitalizations_pct_long$WeekNo-3
rm(list=ls(pattern="temp")) # remove all temporary tables

# Transform demographic data table, convert from short to long-form
DemoChars_Chicago_pct_long <- DemoChars_Chicago_pct %>% gather("Category", "Rate_Ref", 4:18) %>% select(Category,Rate_Ref)

# Reorder label values so that they appear in order within dashboard serial charts
DemoChars_Chicago_pct_long <- DemoChars_Chicago_pct_long %>% mutate(Order=row_number())
...
# Chicago_COVID19_CasesDeathsHospitalizations_pct_long_wk <- Chicago_COVID19_CasesDeathsHospitalizations_pct_long_wk %>% mutate(Label=Category, Label = recode(Label, "Age0_17"="0-17", "Age18_29"="18-29","Age30_39"="30-39", "Age40_49"="40-49", "Age50_59"="50-59", "Age60_69"="60-69", "Age70_79"="70-79", "Age80Pl"="80+", "AsianNL"="Asian","BlackNL"="Black","WhiteNL"="White","OthRaceNL"="Other"))

# write "Disparity_timeseries_metrics.csv" table to be imported into ArcGIS online 
